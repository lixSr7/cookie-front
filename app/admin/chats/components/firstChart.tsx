...
export const ChatsLineChart: React.FC = () => {
  ...
  const [chartData, setChartData] = useState<{
    labels: string[];
    datasets: { label: string; data: number[]; borderColor: string; backgroundColor: string; borderWidth: number }[];
  }>({
    labels: [],
    datasets: []
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        combinedData.sort((a, b) => (b.chats + b.messages) - (a.chats + a.messages));
